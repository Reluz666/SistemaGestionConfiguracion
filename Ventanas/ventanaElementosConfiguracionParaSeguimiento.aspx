    ...
    <script lang="javascript" type="text/javascript">
        ...
        function filtrarTabla(texto) {
            ...
            for (var i = 0; i < datosCompletos.length; i++) {
                ...
                var textoFila = row.NOMBRE_CI + ' ' + row['NOMBRE TIPO CI'] + ' ' + row['NRO SERIE'] + ' ' +
                    row['FABRICANTE / PROVEEDOR'] + ' ' + row.MARCA + ' ' + row.MODELO + ' ' + row['ESTADO CI'] + ' ' +
                    row.PROPIETARIO + ' ' + row['DESCRIPCION CI'] + ' ' + row['IMPACTO CI'] + ' ' +
                    row.SEDE + ' ' + row.LOCAL + ' ' + row['UBICACION LOCAL'] + ' ' + row['DIRECCION LOCAL'] + ' ' +
        ...
        function renderizarTabla(datos) {
            ...
            if (datos.length === 0) {
                tbody.innerHTML = '<tr><td colspan="19" class="text-center text-muted py-4">No se encontraron elementos</td></tr>';
                return;
            }
            ...
            for (var i = 0; i < datos.length; i++) {
                ...
                sb += '<td>' + htmlEncode(row['FABRICANTE / PROVEEDOR']) + '</td>';
                sb += '<td>' + htmlEncode(row.MARCA) + '</td>';
                sb += '<td>' + htmlEncode(row.MODELO) + '</td>';
                ...
                sb += '<td>' + htmlEncode(row['IMPACTO CI']) + '</td>';
                ...
                sb += '<button type="button" class="btn btn-success btn-sm btn-accion" onclick="ELEMENTO_CONFIGURACION(\'' + htmlEncode(row['ID CI']) + '\',\'' + htmlEncode(row.NOMBRE_CI) + '\',\'' + htmlEncode(row['NOMBRE TIPO CI']) + '\',\'' + htmlEncode(row['NRO SERIE']) + '\',\'' + htmlEncode(row['FABRICANTE / PROVEEDOR']) + '\',\'' + htmlEncode(row.MARCA) + '\',\'' + htmlEncode(row.MODELO) + '\',\'' + htmlEncode(row['ESTADO CI']) + '\',\'' + htmlEncode(row['DESCRIPCION CI']) + '\',\'' + htmlEncode(row.SEDE) + '\',\'' + htmlEncode(row.LOCAL) + '\',\'' + htmlEncode(row['UBICACION LOCAL']) + '\',\'' + htmlEncode(row['DIRECCION LOCAL']) + '\',\'' + htmlEncode(row.AREA) + '\',\'' + htmlEncode(row['NRO PISO']) + '\',\'' + htmlEncode(row['NRO AMBIENTE']) + '\')">CI</button>';
                sb += '</td>';
                sb += '</tr>';
            }
