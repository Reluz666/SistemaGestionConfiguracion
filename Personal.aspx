        ...
        function seleccionarPersonal(idPersonal) {
            ...
            for (var i = 0; i < datosPersonalJson.length; i++) {
                if (datosPersonalJson[i].ID_PERSONAL === idPersonal) {
                    ...
                    // For Local and Area, we need to trigger ASP.NET postback cascade
                    // The __doPostBack function is auto-generated by ASP.NET for postbacks
                    try {
                        // Trigger Sede change to load Locales via ASP.NET postback
                    ...
                    if (formCards.length > 1) {
                        formCards[1].scrollIntoView({ behavior: 'smooth' });
                    }
                    return;
                }
            }
